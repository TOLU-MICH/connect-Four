...
for (let i = 0; i < squares.length; i++) {
  // add onclick to each square in your code
  squares[i].onclick = () => {
...
// make const that shows the winning array
const winningArrays = [
  [0, 1, 2, 3],
  [41, 40, 39, 38],
  [7, 8, 9, 10],
...
// add an addEventListener to each square that will trigger the checked functions on click
squares.forEach((square) => square.addEventListener("click", checkBoard));

//check the board for the wins or lose
function checkBoard() {
  for (let i = 0; i < winningArrays.length; i++) {
    // now take the 4 value in each winningArrays and plug time into the squares
